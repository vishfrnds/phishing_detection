...
public class URL_Utilities {
    ...
    static int checkURL(String _url)
    {
        ...
        if(outOfPositionTLD(_url))
            ans += 40;
        ans += senstiveWords(_url)*20;
        return ans;
    }
    
    public static int perMatch(String _url)// to find percentage match with given list of URL
    {
        int l = _url.length(), high = phish_list.length - 1, low = 0, mid, i, flag, ans = 0, maxi = 0;
        while(high >= low)
        {
            flag = 0;
            mid = (high + low) / 2;
            //System.out.println(phish_list[mid]+" "+high+" "+" "+mid +" "+low);
            int len = phish_list[mid].length();
            for(i = 0; i < Math.min(l, len); i++)
            {
                if(_url.charAt(i) != phish_list[mid].charAt(i))
                {
                    flag = 1;
                    break;
                }
        ...
        int per = (int)(maxi*100.0/phish_list[ans].length());
        return per;
    }
    ...
    static int suspiciousURL(String _url)
    {
        ...
        {
            if(_url.charAt(i) == '@' || _url.charAt(i) == '-')
            {
                ctr++;
            }
        }
